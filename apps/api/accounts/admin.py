...
class UserAdmin(BaseUserAdmin):
    ordering = ("-created_at",)
    list_display = ("email", "wechat_openid", "role", "is_email_verified", "is_staff", "is_active", "created_at")
    list_filter = ("role", "is_email_verified", "is_staff", "is_active")
    search_fields = ("email", "wechat_openid", "full_name", "nickname")
    readonly_fields = ("id", "created_at", "updated_at", "last_login", "email_verified_at")
    fieldsets = (
        ("Identity", {"fields": ("id", "email", "wechat_openid", "password")}),
