...
class DatingCandidateListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return DatingProfile.objects.select_related("user").filter(is_visible=True).exclude(user=self.request.user)
...
class DatingMatchListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return DatingMatch.objects.select_related("user_a", "user_b").filter(Q(user_a=self.request.user) | Q(user_b=self.request.user))
