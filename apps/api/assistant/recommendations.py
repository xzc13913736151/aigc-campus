...
def detect_recommendation_kind(prompt: str, page_type: str = "") -> str:
    clean = clean_prompt(prompt)
    if not any(word in clean for word in ("推荐", "匹配", "找几个", "看看", "合适", "智能")):
        return ""
    if any(word in clean for word in ("组队", "队友", "项目", "比赛", "招募")):
        return "team_recommendations"
    if any(word in clean for word in ("恋爱", "对象", "心动", "脱单", "缘分")):
        return "dating_recommendations"
    if any(word in clean for word in ("交易", "闲置", "出售", "求购", "交换", "商品")):
        ...
        items.append(
            {
                ...
                "reason": f"与你输入的关键词相关：{'、'.join(reason_terms[:3])}" if reason_terms else "这条组队与本次需求相关，且仍在开放招募。",
...
def build_dating_recommendations(user, session, prompt: str) -> dict[str, Any] | None:
    ...
    profiles = DatingProfile.objects.select_related("user").filter(is_visible=True).exclude(user=user).exclude(user_id__in=skipped)[:50]
    items: list[dict[str, Any]] = []
        ...
        items.append(
            {
                ...
                "score_label": _score_label(score),
                "reason": (
                    f"符合你的偏好，也命中本次关键词：{'、'.join(prompt_common)}"
                    if prompt_common
        ...
        items.append(
            {
                ...
                "score": score,
                "score_label": _score_label(score),
                "reason": f"与你输入的关键词相关：{'、'.join(matched)}" if matched else "这条交易与本次需求相关，适合先咨询。",
...
def build_recommendation_action(user, session, prompt: str) -> tuple[str, list[dict[str, Any]], dict[str, Any]] | None:
    if is_blocked_prompt(prompt):
        return ("这些操作风险比较高，我不能替你删除、拉黑、隐藏会话或绕过确认执行。", [], {"flow": "idle"})
    kind = detect_recommendation_kind(prompt, session.page_type)
    builders = {
    ...
    return ("我从当前真实数据里挑了几项更合适的推荐，并给了推荐指数。你可以先看看，再选择联系或填充下一步。", [action], {"flow": "ready", "intent": kind})
